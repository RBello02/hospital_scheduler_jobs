...
class Rooms:
    ...
    def remove_patient(self, room_index, solution , patient, starting_time, T):
        if solution.patient_schedule[patient.id]['day'] == starting_time and solution.patient_schedule[patient.id]['room'] == room_index:
            for t in range(starting_time, min(starting_time + patient.length_of_stay, T)):         # i'll consider the min because i don't want to consider a time that exceed T
